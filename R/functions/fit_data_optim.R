fit_data_optim <- function(parameters_included,list_data,likelihood_file,more=F,initial.height){
  ...
  data_explanatory_formated <- format_data_explanatory_updated(
  ...
  # get selection_metric (BIC)
      ...
      selection_metric <- 2*fit$value+dim(param_range)[1]*log(dim(list_data$data_age_height)[1]) # bic # fit$objective is "-sum(log_likelihood)
  ...
  if(more==T){ #return more info (useful in the case we study only the best model)
    ...
    return(data.table(
      ...
      conv.info = ifelse(is.null(fit$message),NA,fit$message)
    ))
