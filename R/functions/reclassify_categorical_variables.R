reclassify_categorical_variables <- function(db,variable,min_pct_per_group,min_abs_per_group){
  ...
  if(length(unique(db_number$group_corrected)) == 1){ # then we remove the variable because not varying after reclassification
