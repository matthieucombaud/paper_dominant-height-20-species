plot_CCimpact_intraspecific <- function(
  ...
  # take mean climate of the initial period
  ...
  # enter species specific code
    ...
    boxplot_impact_climate <- lapply(species_list_ordered, function(species.selected){
      ...
      db_impact_species.selected_summary[, ":="( # add whiskers
      ...
      boxplot_impact_climate.selected <- ggplot(
        ...
        theme(
          axis.text.x = element_text(angle = angle.x, size = text.size.axis, hjust = 1, vjust = 1),
    ...
    graph.arranged <- plot_grid(
      plotlist = lapply(boxplot_impact_climate[1:18], function(x){
        x <- x +
          theme(
    ...
    draw_label(label.x, x = 0.5, y = 0, hjust = 0.5, vjust = 1, angle = 0, size = 1.2 * text.size.main) +
    draw_label(label.y, x = 0, y = 0.5, hjust = 0.5, vjust = 0, angle = 90, size = 1.2 * text.size.main) +
