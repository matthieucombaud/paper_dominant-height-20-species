compute.optimism <- function(
  ...
  db.quality <- rbindlist(lapply(list.species, function(sp.sel){
    ...
    list_model_sel <- list_model.batch[[sp.sel]] # list of the 5 models for the selected species
    ...
    db.quality.sp <- rbindlist(lapply(1:5, function(batch.sel){
      ...
      db_Hp_comparison.sel <- merge(
        ...
        db.Hp_pred.sel[, .(stand, Hp_pred = Hp, year_observation = year)],
      ...
      # attention, at this stage separate calibration and validation to compute mean over stands
      ...
      db.quality[, optimism := max(0, rmse.valid - rmse.calib) / rmse.calib]
