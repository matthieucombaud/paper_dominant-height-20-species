analyze.rmse.bias <- function(
  ...
  # compute calibration and validation rmse per batch
    ...
    list_metrics.submodel <- rbindlist(lapply(names(model_annex), function(name_model_annex.selected){
    ...
    bias.calibration <- db_height.final[stand %in% stands.calibration, mean(height.simulated - height.observed)]
    bias.validation <- db_height.final[stand %in% stands.validation, mean(height.simulated - height.observed)]
    
    bias.abs.calibration <- db_height.final[stand %in% stands.calibration, mean(abs(height.simulated - height.observed))]
    bias.abs.validation <- db_height.final[stand %in% stands.validation, mean(abs(height.simulated - height.observed))]
    ...
    return(
      data.table(
        ...
        bias.calibration = bias.calibration,
        bias.validation = bias.validation,
        bias.abs.calibration = bias.abs.calibration,
        bias.abs.validation = bias.abs.validation
      )
    )
  ...
  # compute calibration rmse for the whole model ----
    ...
    # compute rmse, bias and absolute
    rmse_model.main <- sqrt(db_height.final_model.main[, sum((height.observed - height.simulated)^2)] / nrow(db_height.final_model.main))
    
    bias_model.main <- db_height.final_model.main[, mean(height.simulated - height.observed)]

    bias.abs_model.main <- db_height.final_model.main[, mean(abs(height.simulated - height.observed))]
    
    list_metrics.model.main <- list(
      rmse_model.main = rmse_model.main,
      bias_model.main = bias_model.main, 
      bias.abs_model.main = bias.abs_model.main
    )
  ...
  return(
    list(
      species_code = species_code,
      list_metrics.submodel = list_metrics.submodel,
      list_metrics.model.main = list_metrics.model.main
    )
  )  
