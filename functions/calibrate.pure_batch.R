calibrate.pure_batch <- function(
  sp,
  batch,
  nb_batch=NULL, # only usefull if batch == "with_batches"
  ...
  if(length(group_CV) == 1){
    
    cat(dput(1), file = paste0(output_folder,"/before_calib_",svg_name,"_batch_", "main", ".txt"))
    ...
    cat(dput(1), file = paste0(output_folder,"/after_calib_",svg_name,"_batch_", "main", ".txt"))
    ...
    output <- list(
      ...
      initial.height=initial.height,
      likelihood_file=likelihood_file,
      variable_selection=calibration$variable_selection,
      best.model = calibration$best_model,
      db_parameter_normalized=calibration$best_model$parameter_report_final
    )
    
    print(paste0("end for batch ","main"))
  ...
  if(length(group_CV ) > 1){
    ...
    required_packages<-c("TMB", "data.table", "stringr", "ggplot2", "psych", "tmbstan")
    ...
    output <- foreach(group_CV_k=group_CV,.packages=required_packages,.export=required_functions) %dopar% {
      ...
      output_k <- list(
        ...
        initial.height=initial.height,
        likelihood_file=likelihood_file,
        variable_selection=calibration_k$variable_selection,
        best.model = calibration_k$best_model,
