simulate_density_gDH_ST <- function(
  ...
  # db.modeled <- rbindlist(lapply(stand.all, function(stand.selected){ # compute dynamics in mixed stands for each stand, assuming a given valu of the parameters. 
    ...
    # get species traits
      ...
      # trait used for the focal species
      trait_foc_1 <- as.numeric(db_trait[code == sp1.selected, trait.foc.selected, with = FALSE])
      trait_foc_2 <- as.numeric(db_trait[code == sp2.selected, trait.foc.selected, with = FALSE])
      
      # trait used for the companion species
      trait_comp_1 <- as.numeric(db_trait[code == sp1.selected, trait.comp.selected, with = FALSE])
      trait_comp_2 <- as.numeric(db_trait[code == sp2.selected, trait.comp.selected, with = FALSE])
    ...
    for(year.selected in c(c(t_ori_youngest + 1): t_obs)){ # iterate over year (need to have a doulbe "c", else problem in the vector of year)
      ...
      ME_1 <- (
        par["mu"] + par["alpha"] * gDH_1 +
          par["beta_foc"] * trait_foc_1 + par["beta_comp"] * trait_comp_2 + 
          par["gamma_foc"] * trait_foc_1 * gDH_1 + par["gamma_comp"] * trait_comp_2 * gDH_1
      )* (1 - prop_sp1)
      ...
      ME_2 <- (
        par["mu"] + par["alpha"] * gDH_2 +
          par["beta_foc"] * trait_foc_2 + par["beta_comp"] * trait_comp_1 + 
          par["gamma_foc"] * trait_foc_2 * gDH_2 + par["gamma_comp"] * trait_comp_1 * gDH_2
      )* (1 - prop_sp2)
