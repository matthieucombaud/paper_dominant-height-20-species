...
list(
  ...
  # prepare and write the db to be used for inference ----
    ...
    # list species to keep
  
    tar_target(list_species.keep,
               c("03", "51", "02", "09", "52", "64", "62", "05", "61", "53CO", "10", "54", "17C", "57A", "63", "14", "73", "04", "12V", "11")
               ),
  
  
    # filter db for inference (keep only the first 20)
    tar_target(
      list_db_species_for_inference_keep, 
      keep_db_species_for_inference(
        list_db_species_for_inference_all = db_species_for_inference_list,
        list_species.keep = list_species.keep
      )
    ),
  
  
    # get only normalization constant
    tar_target(
      list_normalization_constant,
      get_normalization_constant(
        list_inference = list_db_species_for_inference_keep
      )
    ),
    ...
    # tar_target(
    #   list_tab.categorical.variables, 
    #   get_tab.categorical.variables(
    #     list_inference = db_species_for_inference_list
    #   )
    # ),
    ...
    tar_target(db.pure.stand.calib,
               get.list.db.pure.stand(
                 list_data.inference = list_db_species_for_inference_keep
               )),
  
  
    # db.envi for pure stand used for calibration
    tar_target(db.envi.pure.stand,
               db.envi[stand %in% unique(db.pure.stand.calib$stand)]
               ),
    ...
    tar_target(
      ...
      get_short.names(
        ...
        species_list = list_species.keep
      )
    ),
